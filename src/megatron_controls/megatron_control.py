...
class MegatronControl:
    ...
    def __call__(self, command, args, current_script_path=None):
        command_dispatcher = {
            "email": self.email,
            "exit": self.exit_command,
            "failif": self.failif,
            "failifoff": self.failifoff,
            "l": self.l_command,
            "log": self.log,
        ...
        if command in command_dispatcher:
            ...
            kwargs = {"args": args, "current_script_path": current_script_path}
    ...
    def lograte(self, args):
        try:
            ...
            if hasattr(self._context, "logging_stop_event") and self._context.logging_stop_event:
                print("Stopping existing logging...")
                self._context.logging_stop_event.set()

            self._context.logging_stop_event = asyncio.Event()

            async def logging_coro():
                await asyncio.sleep(0)
                stop_event = self._context.logging_stop_event
                while not stop_event.is_set():
                    is_new_file = False
                    log_file_path = self._context.log_file_path
                    signals = self._context.logged_signals
    ...
    def failif(self, args):
        ...
        device_name = self._context.device_mapping.get(pv_name)
        if not device_name:
            raise RuntimeError(f"PV {pv_name} not found in device mapping.")

        pv_signal = getattr(self._context.devices, device_name)
        if not pv_signal:
        ...
        def check_pv_value(value, **kwargs):
            if value == expected_value:
                ...
                called_script_path = os.path.join(self._context.script_dir, fail_script)
                self._context.run_script_callback(called_script_path)
    ...
    def plot(self, args):
        if len(args) == 1 and args[0].lower() == "dump":
            ...
            for pv_name, signal in self._context.logged_signals.items():
        ...
        if not os.path.isfile(self._context.log_file_path):
            print("Error: Log file does not exist. Please ensure logging is enabled.")
            yield from bps.null()
            return

        try:
            df = pd.read_csv(self._context.log_file_path, comment=None, header=0, skip_blank_lines=True)
        ...
        plot_dir = os.path.join(self._context.logging_dir, "plots")
    ...
    def run(self, args):
        ...
        called_script_path = os.path.join(self._context.script_dir, script_name)
    ...
    def waitai(self, args):
        ...
        if source in self._context.device_mapping:
            device_name = self._context.device_mapping[source]
            signal = getattr(self._context.devices, device_name)
        else:
